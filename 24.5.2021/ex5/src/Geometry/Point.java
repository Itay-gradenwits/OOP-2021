...
public class Point {
    ...
    /**
     * matchCollisionPointTocollidableObject -- find a collidable object by given the collision point of one of the
     * collidable objects in a list with a line.
     *
     * @param cList the list of the collidable objects
     * @param line  the line that represent the movement of an object that collision with one of the collidable Objects
     *              at this point.
     * @return the collidable object that the collision point is on it.
     */
    public Collidable matchCollisionPointToCollidableObject(List<Collidable> cList, Line line) {
        Collidable collidable = null;
        if (cList == null) {
            throw new RuntimeException("cList in not initialized");
        }
        // search for a collidable object that its closest collition point is equal to this point
        for (Collidable c : cList) {
            if (line.closestIntersectionToStartOfLine(c.getCollisionRectangle()) != null) {
                if (line.closestIntersectionToStartOfLine(c.getCollisionRectangle()).equals(this)) {
                    collidable = c;
                    break;
                }
            }
    ...
    /**
     * findGoodMovePoin -- by given an intersection point we find a new point that is on the line that represent
